...
Node merge(Node l, Node r) {
  return {
    l.sum+r.sum,
    max(l.maxPrefixSum, l.sum+r.maxPrefixSum),
    max(r.maxSuffixSum, l.maxSuffixSum+r.sum),
    max(l.sum+r.sum, max(l.maxPrefixSum, max(l.sum+r.maxPrefixSum, max(r.maxSuffixSum, max(l.maxSuffixSum+r.sum, max(l.ans, max(r.ans, l.maxSuffixSum+r.maxPrefixSum))))))),
  };
}
...
int main() {
  ...
  vector<Node> segmentTree(nearestPow*2, {0, 0, 0, 0});
  for (ll i = 0; i < n; i++) {
    ll cur; cin >> cur;
    segmentTree[nearestPow + i] = {cur, cur, cur, cur};
  }
  ...
  for (ll i = 0 ; i < q; i++) {
    ...
    if (z == 'Q') {
      ...
      cout << finalRes.ans << endl;
    } else if (z == 'S') {
      ll idx, x;
      cin >> idx >> x;
      ll upd = nearestPow+idx-1;
      segmentTree[upd] = {x, x, x, x};
      upd/=2;
      while (upd >= 1) {
